...
fad install <ID> -s SERIAL  # Install to a specific device (-s / --serial)
...
- APK releases are installed directly with `adb install -r`. With several devices connected, adb needs to know which one to use: pass `-s` / `--serial` (forwarded as `adb -s SERIAL`) or set `ANDROID_SERIAL`
